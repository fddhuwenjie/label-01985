  ...
  <div>
    <el-card>
      <div style="display: flex; justify-content: space-between; margin-bottom: 16px">
        <el-input v-model="keyword" placeholder="搜索教师姓名/工号" style="width: 300px" clearable @clear="loadData">
          <template #append>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑教师' : '新增教师'" width="500px"
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="职称">
          <el-input v-model="form.title" placeholder="请输入职称，如：教授、副教授" maxlength="30" />
        </el-form-item>
...
const form = reactive({ id: null, teacherNo: '', name: '', gender: 1, title: '', phone: '' })
const rules = {
  teacherNo: [
    { required: true, message: '请输入工号', trigger: 'blur' },
    { min: 2, max: 30, message: '工号长度为2-30个字符', trigger: 'blur' }
  ],
  name: [
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await getTeacherPage({ current: current.value, size: size.value, keyword: keyword.value })
...
const openDialog = (row) => {
  if (row) {
    Object.assign(form, { id: row.id, teacherNo: row.teacherNo, name: row.name, gender: row.gender, title: row.title, phone: row.phone })
  } else {
    Object.assign(form, { id: null, teacherNo: '', name: '', gender: 1, title: '', phone: '' })
  }
...
const handleDelete = async (id) => {
  try {
    await ElMessageBox.confirm('确定要删除该教师吗？删除后不可恢复。', '删除确认', {
